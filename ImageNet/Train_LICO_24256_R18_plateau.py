...
def train(args, logger):
    ...
    if args.scheduler_type == 'plateau':
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode='min',factor=0.1,patience=2,min_lr=0,eps=1e-8,threshold_mode='rel')
    ...
    # 将学习率列表写入txt
    for id,item in enumerate(lr_list):
        lr_list[id] = str(item)
    f = open("lr_list.txt","w")
    str = '\n'
    f.write(str.join(lr_list))
    f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_model_path', type=str,
                        default='save_model/R18/LRQ/quantization_2_4_k256_compressed_model_imagenet_df15_plateau_trained.pt')
