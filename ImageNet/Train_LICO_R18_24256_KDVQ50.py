...
def train(args, logger):
    ...
    teacher_model = torch.load('save_model/R50/LRQ/quantization_2_4_k256_compressed_model_imagenetd_nodf_e-4_trained.pt')
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        for batch_id,(images, labels) in enumerate(trainloader):
            ...
            student_logits = F.log_softmax(preds/args.temperature, dim=1)
            # calculate teacher logits
            with torch.no_grad(): teacher_logits = F.softmax(teacher_model(images)/args.temperature, dim=1)
            loss = 0.6 * pow(args.temperature,2) * F.kl_div(student_logits, teacher_logits,reduction='batchmean') + 0.4 * F.cross_entropy(preds, labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_model_path', type=str,
                        default='save_model/R18/LRQ/quantization_2_4_k256_compressed_model_imagenet_df15.pt')
    parser.add_argument('--save_model_path', type=str,
                        default='save_model/R18/LRQ/quantization_2_4_k256_compressed_model_imagenet_df15_standardKDVQ50_trained.pt')
