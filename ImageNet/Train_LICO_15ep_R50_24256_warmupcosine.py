...
from lr_scheduler_utils.lr_scheduler import get_adam_optimizer,\
    get_SGD_optimizer,get_milestone_scheduler,get_cosine_annealing_scheduler,get_WarmupCosine_scheduler
...
def train(args, logger):
    ...
    if args.scheduler_type == 'warmupcosine':
        optimizer = get_adam_optimizer(quantization_model, initial_lr=args.warmup_initial_lr)
        logger.logger.info('Scheduler_type is warmupcosine')
        scheduler = get_WarmupCosine_scheduler(optimizer=optimizer, epochs=args.num_epoch,
                                               eta_min=args.warmup_initial_lr,
                                               eta_max=args.warmup_eta_max, warm_up_ep=args.warmup_ep)
    ...
    logger.logger.info('Initial learning rate is'+str(args.initial_lr))
    test_acc_list = []
    for epoch in range(args.num_epoch):
        ...
        for batch_id,(images, labels) in enumerate(trainloader):
            ...
            logger.logger.info('Epoch:[{}/{}]\t loss={:.5f}\t batch:[{}/{}]\t acc={:.3f}%'.format(epoch, args.num_epoch, loss, batch_id,
        ...
        test_acc = test(quantization_model,args)
        test_acc_list.append(test_acc)
        logger.logger.info(
            'Epoch:[{}/{}]\t loss={:.5f}\t train_acc={:.3f}%\t test_acc={:.3f}%'.format(epoch, args.num_epoch, train_loss, present_trainset_acc * 100, test_acc * 100))
        scheduler.step()
    logger.logger.info('Done training!')
    logger.logger.info('The best performance is:{:.3f}%'.format(max(test_acc_list) * 100))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_model_path', type=str,
                        default='save_model/R50/LRQ/quantization_2_4_k256_compressed_model_imagenetd_nodf.pt')
    parser.add_argument('--save_model_path', type=str,
                        default='save_model/R50/LRQ/quantization_2_4_k256_compressed_model_imagenetd_nodf_warmupcosine_trained.pt')
    ...
    parser.add_argument('--warmup_eta_max',type=float,default=1e-5)
    parser.add_argument('--warmup_ep',type=int,default=3)
