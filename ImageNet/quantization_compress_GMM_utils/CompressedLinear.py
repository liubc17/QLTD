...
class CompressedLinear(AbstractCompressedLayer):
    ...
    def from_uncompressed(
        ...
        assert kmeans_fn in [kmeans, src, GMM]
        ...
        codebook, codes = kmeans_fn(training_set, k=num_centroids, n_iters=k_means_n_iters)
        codes_matrix = codes.reshape(-1, num_blocks_per_row * num_centroids)
