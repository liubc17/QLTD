...
def train(args, logger):
    ...
    logger.logger.info('This is artifcial LR training, ep = {}.'.format(args.num_epoch))
    ...
    test_acc_list = []
    artificial_lr = [pow(10,-4.6),pow(10,-4.6),pow(10,-4.6),pow(10,-5),pow(10,-5.5)]
    for epoch in range(5, args.num_epoch + 1):
        optimizer.param_groups[0]['lr'] = artificial_lr[4]
        ...
        for batch_id, (images, labels) in enumerate(trainloader):
            ...
            logger.logger.info(
                'Epoch:[{}/{}]\t loss={:.5f}\t batch:[{}/{}]\t acc={:.3f}%'.format(epoch, args.num_epoch, loss,
                                                                                   batch_id + 1,
                                                                                   len(trainloader),
                                                                                   preds.argmax(dim=1).eq(
                                                                                       labels).sum().item() / len(
                                                                                       images) * 100))
        ...
        logger.logger.info(
            'Epoch:[{}/{}]\t loss={:.5f}\t train_acc={:.3f}%\t test_acc={:.3f}%'.format(epoch, args.num_epoch,
                                                                                        train_loss,
                                                                                        present_trainset_acc * 100,
                                                                                        last_acc * 100))
    logger.logger.info('Done training!')
    checkpoint = {
        "net":quantization_model.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_model_path', type=str,
                        default='Find_lr_utils/Artificial_lr/quantization_1.6_2_k256_compressed_model_imagenet_df30_artificial_ep5.pth')
