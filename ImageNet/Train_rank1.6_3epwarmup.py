...
def train(args, logger):
    ...
    if args.scheduler_type == 'None':
        logger.logger.info('Scheduler_type is None')
    ...
    for epoch in range(args.num_epoch):
        ...
        for batch_id,(images, labels) in enumerate(trainloader):
            ...
            logger.logger.info('Epoch:[{}/{}]\t loss={:.5f}\t batch:[{}/{}]\t acc={:.3f}'.format(epoch, args.num_epoch, loss, batch_id,
                                                                                                 len(trainloader),preds.argmax(dim=1).eq(labels).sum().item() / len(images)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_model_path', type=str,
                        default='save_model/R18/lowrank/lowrank_1.6_compressed_model_imagenet.pt')
    parser.add_argument('--save_model_path', type=str,
                        default='save_model/R18/lowrank/lowrank_1.6_compressed_model_imagenet_3epwarmup.pt')
