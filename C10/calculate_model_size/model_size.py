...
def compute_model_nbits(model: torch.nn.Module) -> int:
    ...
    for module_name, module in compressed_dict.items():
        ...
        if 's_weights' in module_name:
            n_parameters = torch.nonzero(module).size(0)  # 计算稀疏项非零个数
        elif 'mask' in module_name:
            n_parameters = 0
        else:
        ...
        #     if p.requires_grad:
        #         ids = torch.nonzero(p)
        # n_parameters = sum([torch.nonzero(p).size(0) for p in module if p.requires_grad])
